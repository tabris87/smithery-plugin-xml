...
function enrichTree(xmlTree, oOptions) {
    ...
    if (oOptions && oOptions.parent) {
        ...
        //case of the empty document without declaration
        if(xmlTree.elements && xmlTree.elements.length > 0 && typeof xmlTree.type === "undefined") {
            debugger;
        }
    ...
    if (xmlTree.attributes && xmlTree.attributes.id) {
        /* returnNode.name = returnNode.name + '_' + xmlTree.attributes.id; */ //check this out O_O
        returnNode.name = xmlTree.attributes.id;
    }
