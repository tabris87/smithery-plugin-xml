...
function checkIDChild(oFST) {
    return oFST.elements && oFST.elements.filter(oChild => oChild.attributes && oChild.attributes.some(oAtt => oAtt.identifier.name === "id")).length > 0;
}
...
const rule = {
    ...
    target: ['xml'],
    selector: 'document[elements], element[elements], declaration[attributes], element[attributes]'
};
